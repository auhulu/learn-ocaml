...
let rec insert_half tree ekimei1 ekimei2 kyori = 
    let tmp_lst = try search tree ekimei1 with Not_found -> [] in insert tree ekimei1 ((ekimei2, kyori)::tmp_lst)
...
let rec get_ekikan_kyori ekimei1 ekimei2 tree = 
    let tmp_lst = try search tree ekimei1 with Not_found -> raise Not_found
...
(* string型のリストを受け取り逆順に1行ずつ表示する *)
(* print_reverse_list: string list -> unit *)
let rec print_reverse_list lst =  match lst with
    | [] -> ()
    | first :: rest -> (print_reverse_list rest; print_string first; print_string "\n")

(* eki_t型のレコードを受け取りいい感じに表示する *)
(* print_eki: eki_t -> unit *)
let print_eki {namae=n;saitan_kyori=s;temae_list=t} = 
    (
        print_string n; print_string "駅までの最短距離は"; print_float s; print_string "kmです\n";
        print_string "最短経路は以下の通りです\n";
        print_reverse_list t
    )

(* メイン関数 *)
(* 起点と終点を受け取り最短距離、経路をいい感じに表示 *)
(* main: string -> string -> unit *)
let main kiten shuten = print_eki (dijkstra kiten shuten)

(* メイン関数の呼び出し *)
(* $ ./metro.top "shibuya" "gokokuji" *)
